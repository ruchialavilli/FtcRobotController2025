...
public class GOATEDNEW extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            while (gamepad1.right_bumper) { //in
                armExtension.setPower(1);
            }
            while (gamepad1.left_bumper) { //out
                armExtension.setPower(-1);
            }    // Arm extension control

            if (gamepad1.dpad_down) { //pick
                intakePitch.setPosition(0.5);
            }
            if (gamepad1.dpad_up) { //drop
                intakePitch.setPosition(0.7);
            }
            if (gamepad1.y) { //triangle //outtake
                intakeLeft.setPower(-1);
                intakeRight.setPower(1);
            }
            if (gamepad1.x) { //square //intake
                intakeLeft.setPower(1);
                intakeRight.setPower(-1);
            }
